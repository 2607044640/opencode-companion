...
describe('Curated Model Filter Engine', () => {
  test('ALLOWS canonical approved models', () => {
    ...
    // 3. free-auto-cascade (Free Cascade connected to OpenCode)
    assert.equal(isCuratedModel('obsidian', 'free-auto-cascade'), true)
    assert.equal(isCuratedModel('obsidian', 'free-cascade'), true)
    assert.equal(isCuratedModel('custom', 'free-auto-cascade', '智能级联 (Free)'), true)

    // 4. gemini-3.7-flash (Free Flash connected to OpenCode)
  ...
  test('canonical model IDs set contains the expected 4 IDs', () => {
    ...
    assert.ok(ALLOWED_CANONICAL_MODEL_IDS.has('free-auto-cascade'))
